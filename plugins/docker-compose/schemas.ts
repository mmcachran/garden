...
const commonRunKeys = s.object({
  ...
  arguments: s.array(s.string()).describe("Arguments to pass to the command."),
  env: s.envVars().describe("Environment variables to set during execution."),
  user: s.string().describe("Run the command as this user."),
  workdir: s.string().describe("Path to workdir directory for this command."),
})
